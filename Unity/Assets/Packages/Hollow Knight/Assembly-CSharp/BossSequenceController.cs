...
public static class BossSequenceController
{
	...
	public static int BoundNailDamage
	{
		get
		{
			...
			if (@int <= currentSequence.nailDamage)
			{
				return Mathf.RoundToInt((float)@int * currentSequence.lowerNailDamagePercentage);
			}
	...
	public static void SetupNewSequence(BossSequence sequence, ChallengeBindings bindings, string playerData)
	{
		...
		WasCompleted = false;
		GameManager.instance.playerData.SetVariableSwappedArgs<BossSequenceData>(null, "currentBossSequence");
		SetupBossScene();
	}
	...
	{
		currentData = GameManager.instance.playerData.GetVariable<BossSequenceData>("currentBossSequence");
	...
	public static void ApplyBindings()
	{
		...
		if (BoundCharms)
		{
			currentData.previousEquippedCharms = GameManager.instance.playerData.GetVariable<List<int>>("equippedCharms").ToArray();
			GameManager.instance.playerData.GetVariable<List<int>>("equippedCharms").Clear();
			currentData.wasOvercharmed = GameManager.instance.playerData.GetBool("overcharmed");
			GameManager.instance.playerData.SetBoolSwappedArgs(value: false, "overcharmed");
	...
	public static void RestoreBindings()
	{
		if ((bool)GameManager.instance && (bool)HeroController.instance)
		{
			if (BoundCharms)
			{
				GameManager.instance.playerData.SetVariableSwappedArgs(new List<int>(currentData.previousEquippedCharms), "equippedCharms");
				SetCharmsEquipped(value: true);
				GameManager.instance.playerData.SetBoolSwappedArgs(currentData.wasOvercharmed, "overcharmed");
			}
			HeroController.instance.CharmUpdate();
			if (currentData != null)
			{
	...
	private static void FinishBossScene(BossSceneController self, BossSceneController.SetupEventDelegate setupEvent)
	{
		...
		GameManager.instance.playerData.SetVariableSwappedArgs(currentData, "currentBossSequence");
