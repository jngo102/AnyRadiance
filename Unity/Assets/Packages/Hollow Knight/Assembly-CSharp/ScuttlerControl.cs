...
public class ScuttlerControl : MonoBehaviour, IHitResponder
{
	...
	private IEnumerator Run()
	{
		...
		while (true)
		{
			float num = Mathf.Sign(hero.position.x - transform.position.x) * (float)((!reverseRun) ? 1 : (-1));
			float currentDirection = num;
			transform.SetScaleX(Mathf.Abs(transform.localScale.x) * num);
			while (currentDirection == num)
			{
				...
				yield return null;
				num = Mathf.Sign(hero.position.x - transform.position.x) * (float)((!reverseRun) ? 1 : (-1));
			}
			yield return null;
		}
	}
	...
	{
		source.enabled = false;
		bounceSound.SpawnAndPlayOneShot(audioSourcePrefab, transform.position);
	...
	private IEnumerator Heal()
	{
		...
		GameManager.instance.UnloadingLevel += doHeal;
		if ((bool)HeroController.instance && Vector2.Distance(transform.position, HeroController.instance.transform.position) > 40f)
		{
